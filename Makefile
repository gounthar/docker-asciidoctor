...
README: asciidoctor.build cache/pandoc-$(PANDOC_VERSION)/bin/pandoc
	docker run --rm -t -v $(CURDIR):/documents --entrypoint bash asciidoctor \
		-c "asciidoctor -b docbook -a leveloffset=+1 -o - README.adoc | /documents/cache/pandoc-$(PANDOC_VERSION)/usr/bin/pandoc  --atx-headers --wrap=preserve -t gfm -f docbook - > README.md"
